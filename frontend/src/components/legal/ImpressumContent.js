    ...
    var renderBlocks = function (blocks) {
        ...
        return blocks.map(function (block, blockIndex) {
            if (block.type === 'paragraph') {
                return (<p key={blockIndex} className="mb-3 leading-relaxed">
            {block.children.map(function (child, childIndex) {
                        if (child.type === 'link') {
                            return (<a key={childIndex} href={child.url} className="text-blue-600 hover:underline break-words">
                    {child.text}
                  </a>);
                        }
...
exports.default = ImpressumContent;
